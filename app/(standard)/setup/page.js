    ...
    .then(() => {
      if (!isPanelInProgress) {
        if (isNewComic) {
          ...
          if ((!isSolo && !comicsCreatedInLast24Hours.length) || (isSolo && existingComics.length < 3)) {
            createNewComic(authUser.uid, isSolo);
            router.push("/create");
          } else {
            isSolo ? setError("You have reached your new comic limit. Please complete one of your existing comics before starting a new one!") : setError("You have reached your daily new comic limit. You can only create one new team comic each day. Please contribute to an existing team comic or go solo!");
          }
        } else {
