...
export default function CreateComicPage() {
  ...
  async function continueExistingSolo(uid) {
    ...
    const q = query(collection(db, "comics"), where("createdBy", "==", userRef), where("isSolo", "==", true), where("isCompleted", "==", false));
    const querySnapshot = await getDocs(q);
    const existingSoloComics = querySnapshot.docs.map(doc => ({id: doc.id, ...doc.data()}))
